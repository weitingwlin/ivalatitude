...
% using other indices: MRS, nestedness, turnover
clear; clc
work_compile3
%% Taxon, Euclidean
 [t_r, t_p] = f_mantel(squareform(pdist( t_LdataST)),  squareform(pdist(  latitudeST)), 0, 10000);
 [t_r2, t_p2] = f_mantel(squareform(pdist( t_LdataST)),  squareform(pdist(monthST)), 0, 10000);
...
%% Taxon, MSR
  t_betaMSR = squareform(betaMRS_pairwise(t_matST) );
 ...
 clear t_r  t_p t_r2 t_p2
%%   
 t_betaNES = betaNES_pairwise(t_matST) ;
    %d = mean(betaNES.psim);
...
disp(['Temporal  ', indexstr , ' (taxon) :     r = ', num2str(t_r2, 3), ';  ', pstring(t_p2) ])
   clear t_r  t_p t_r2 t_p2
%% Guild, Euclidean
 [g_r, g_p] = f_mantel(squareform(pdist( g_LdataST)),  squareform(pdist(  latitudeST)), 0, 10000);
 [g_r2, g_p2] = f_mantel(squareform(pdist( g_LdataST)),  squareform(pdist(monthST)), 0, 10000);
 ...
 clear g_r  g_p g_r2 g_p2
   %% guild-based MRS
  g_betaMSR = squareform(betaMRS_pairwise(g_matST) );
 ...
 clear g_r  g_p g_r2 g_p2
%% guild  
 g_betaNES = betaNES_pairwise(g_matST) ;
    %d = mean(betaNES.psim);
